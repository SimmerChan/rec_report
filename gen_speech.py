#!/usr/bin/env python3
"""
Generate speech.md based on slides-v2.html and research documents
Target: 5000-7000 Chinese characters for 30 minutes speech
"""

speech_content = '''# 推荐算法演进与生成式推荐 - 团队汇报演讲稿
...
**腾讯是从上下文工程到广告深融合，** 到GPR到OneRanker，Shopee主搜索GMV提升2%、广告提升2.9%。
...
**看好派**认为：九河之间说字节是极致进化非范式革命，判别式仍是基石；王喆说生成式价值在于突破信息茧房，世界知识注入是核心优势。
...
with open('/Users/huangshilei/Documents/tech/recommendation/algorithm_report/speech.md', 'w', encoding='utf-8') as f:
    f.write(speech_content)

print("speech.md generated successfully!")

# Count characters (excluding markdown formatting)
import re
# Remove markdown syntax for accurate Chinese character count
clean_text = re.sub(r'[#*>\-|]', '', speech_content)
char_count = len(clean_text)
print(f"Total characters (excluding markdown): {char_count}")

# Estimate speaking time
# Chinese speech typically 150-180 chars/min for moderate speed
min_speed = 180
max_speed = 200
min_time = char_count / max_speed
max_time = char_count / min_speed
print(f"Estimated speaking time: {min_time:.1f} - {max_time:.1f} minutes")
