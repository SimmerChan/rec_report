...
- **腾讯OnePiece (2025.9)**：统一Encoder-Decoder架构，Shopee搜索+2% GMV/UU，广告+2.90%
...
腾讯/Shopee的OnePiece论文很有特色，用的是LLM式上下文工程注入推荐，Block-wise隐式推理。但2026年的方向明显转向广告：GPR（端到端生成广告）、OneRanker（generation与ranking架构级协同）。
