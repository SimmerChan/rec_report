...
- **腾讯/Shopee OnePiece (2025.9)**：统一Encoder-Decoder架构，Shopee主搜索+2% GMV/UU，广告+2.90%（注：论文来自Shopee团队，Shopee为腾讯投资的东南亚电商平台）
...
腾讯/Shopee的OnePiece论文很有特色，用的是LLM式上下文工程注入推荐，Block-wise隐式推理。2026年的方向明显转向广告：GPR（端到端生成广告）、OneRanker（generation与ranking架构级协同）。

**Notice**: 这里有个值得关注的转向——腾讯从"上下文工程"转向"广告端到端生成"，某种程度上说明生成式在广告场景的落地比主推荐链路更快。

需要说明的是，OnePiece论文作者团队来自Shopee（腾讯投资的东南亚电商平台），而非腾讯总部。Shopee和腾讯都属于腾讯系，但在推荐系统的技术研发上是独立的。
