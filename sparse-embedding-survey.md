...
#### 4.7.5 知乎用户图谱 · 多数据源交叉网络分析

> **完整报告见**：[`zhihu-user-graph.md`](./zhihu-user-graph.md)（v2 扩展版，22.6 KB）
>
> **2026-06-21 抓取** · 登录账号 SimmerChan · 涉及 **261 位用户**
...
**v2 新增 4 个数据源**：
- ③ GuoXun 关注的 23 人（高可信作者的关注列表）
- ④ 知乎高赞问题「生成式推荐」下 58 位回答者（API 抓取）
- ⑤ GuoXun 的 80 位关注者（反向影响力）
- ⑥ 九老师（nphard-79）的 80 位关注者（**含李沐/周源/苏剑林**等顶级大 V）
...
```
      GuoXun（北大·百度算法）
        ↙           ↘
九老师 (nphard-79)     3473353276
新加坡 / TikTok         美国 / CS PhD
推荐算法工程师            Senior ML Researcher
```

**通过 6 个数据源交叉验证的核心 8 人**（同时在 2+ 源出现）：

| 姓名 | people ID | 出现源数 | 业务价值 |
|------|----------|---------|---------|
| **九老师** | nphard-79 | **3 源**（SimmerChan/GuoXun/GuoXun关注者）| 互关核心 |
| **GuoXun** | guo-xun-16 | 2 源 | 互关核心 |
| **0xC001** | mlpod | 2 源 | 互关核心 |
| **浅梦** | shenweichen | 2 源 | 互关 |
| **Tim 在路上** | lao-zhang-cao-mei-yuan | 2 源 | 北邮硕 |
| **王小桥** | smallqiao | 2 源 | 大连理工硕 |
| **3473353276** | 3472236588 | 2 源 | CS PhD 美国 |
| **nono** | nono-nono-66 | 2 源 | AI 兴趣 |

**九老师关注中 20+ 顶级用户**（v2 新发现）：

| 类别 | 代表用户 |
|------|---------|
| **学术顶流** | 李沐 (亚马逊首席科学家) / 周源 (知乎 CEO) / 苏剑林 (科学空间) / 代季峰 (MSRA) / 黄海广 (机器学习初学者) / 微软亚研院 (官方) |
| **业务一线** | DataMan (抖音 feed) / 0xC001 (阿里) / Hazza (推荐系统与AutoML) / 张备 (推搜广) / 程序员老刘 (老刘聊广告) / momo (推荐算法) |
| **高学历** | 张海抱 (清华博士) / 司马新 (NYU) / 旷野 (埃尔朗根-纽伦堡 AI 硕) / wangyang (中科大博士) / 极简投资人 (北大 CS 硕) |

**关键洞察**：

1. **9 老师是图谱超级节点** — 互关 + 高产 + TikTok 在职，覆盖 80+ 顶级 AI 用户
2. **通过 9 老师关注链 = 通往 李沐 + 周源 + 苏剑林** 的桥梁
3. **SimmerChan 关注中只有 1 互关**（浅梦），但 8 个 AI 用户中**九老师就是 GuoXun 互关**（间接形成"我 ↔ 9 老师 ↔ GuoXun"的三方网络）
4. **v1 建议加关注的 9 人**中，**9 老师 (nphard-79) 实际已通过互关网络被覆盖** — v2 推荐按"互关网络"扩展
5. **抓取方法升级**：用知乎 API (`/api/v4/...`) 替代 DOM extract，速度提升 10x+

**建议 SimmerChan 加关注的 Top 15**（v2 更新）：

1. 浅梦 ✓ 已有 | 2. 王喆 ✓ 已有 | 3. 九老师 ✓ 已有
4. **GuoXun** ⭐⭐⭐⭐⭐ | 5. **0xC001** ⭐⭐⭐⭐⭐ | 6. **傅聪_Cong** ⭐⭐⭐⭐⭐
7. **DataMan** (抖音 feed) | 8. **李见黎** (北大/计算广告) | 9. **Tim 在路上** (北邮硕)
10. **刘聪NLP** (LLM 顶流) | 11. **硅基趣玩喵** (阿里高级) | 12. **木阿木阿木啊** (深耕广告)
13. **王小桥** (大连理工硕) | 14. **程引** (Shopee 推荐) | 15. 苏剑林 (科学空间)
