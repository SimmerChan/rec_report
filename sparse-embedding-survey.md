# 推荐系统 Sparse Embedding 规模与工程方案 · 2025-2026 调研报告

> 编辑于 2026年6月·北京
> **数据来源**：13 篇代表性算法论文原文（8 篇经典判别式 + 5 篇生成式 / 混合）+ 6 篇专门研究稀疏 embedding 工程优化的"系统论文"+ 知乎社区一手调研 + 工业界公开技术博客
> **所有数据均可溯源到 arXiv 原文 + 章节 / Table 编号 / 知乎专栏链接**
> **提取字段**：稀疏特征个数（# features）· 单条 sparse embedding 维度（d）· 总存储空间 · 业务场景 · 工程方案 · 作者可信度
> **最后更新**：2026-06-20
...
在工业级推荐系统论文和实际生产环境里，"sparse embedding" 的实际规模究竟有多大？即：
1. **稀疏特征有多少个**（vocabulary size / feature count）
2. 每条 sparse embedding 的 **维度是多少**（embedding dim）
3. **总存储空间有多大**（GB / TB）
4. 不同业务场景（短视频 / 电商 / 广告 / 社交 / 内容社区）的存储预算
5. 业界用什么 **工程方案**（参数服务器、多级存储、量化、Hash 压缩）解决存储与查询问题

**章节地图**（按推荐阅读顺序）：

| 章节 | 适合谁读 | 内容 |
|------|---------|------|
| 一、形态分类 | 所有人 | sparse embedding 在 2025-2026 推荐系统中的 3 种存在方式（必读基础）|
| 二、13 篇代表性论文汇总 | 算法工程师 / 研究者 | 论文原文的直接数字（13 篇算法的 sparse embedding 表）|
| 三、工业系统论文 | 系统工程师 | 6 篇专门优化 sparse embedding 工程系统的论文（Persia / HugeCTR / Monolith / TT-Rec / AutoShard / Comp Partitions）|
| 四、知乎 + 工业界调研 | 所有人 · **本报告精华**| 业务场景 × 存储空间 × 工程方案 真实落地数据（字节 / 阿里 / 腾讯 / 快手 / 美团 / 百度 / 京东 + 海外）|
| 五、"反常识"数据点 | 想避坑的人 | 6 个广为流传但实际可能被误读的数据点 |
| 六、披露度统计 | 想找研究 gap 的人 | 各论文披露哪些字段，未披露哪些 |
| 七、跨论文速查 | 写论文 / 写报告的人 | 全文关键数据点速查表 |
| 八、结论与建议 | 决策者 | 5 条核心结论 + 4 条后续建议 |

**阅读建议**：
- 想了解 **整体格局** → 看 〇 + 一 + 八
- 想做 **业务容量规划** → 看 四 + 八
- 想做 **学术综述** → 看 二 + 三 + 五 + 六 + 七
- 想做 **系统选型** → 看 四.4 工程方案全景 + 八

**保留原则**：所有数据均来自论文原文或知乎专栏一手引用。**找不到的字段留空，绝不猜测**。每个数据点都标注可信度（论文 / ⭐⭐⭐⭐⭐ 作者 / ⭐⭐⭐⭐ 高 / ⭐⭐⭐ 中 / ⭐⭐ 低）。

---


## 一、按"是否真有 sparse embedding"重新分类

从后面的论文汇总表（第二章）和工业系统论文（第三章）可以清晰看到，工业推荐论文里"sparse embedding"实际有 **三种存在形态**：
...
## 二、13 篇代表性论文 · Sparse Embedding 规模汇总表
...
## 四、知乎 + 工业界调研 · 业务场景 × 存储空间 × 工程方案

> **本节是本报告的精华章节**，回答三个核心问题：
> 1. **业务场景**：我的业务（短视频 / 电商 / 广告 / 社交）到底需要多大 sparse embedding 存储？
> 2. **存储空间**：业界公开的标尺数据是多少？
> 3. **工程方案**：用什么方法解决存储和查询问题？

---

### 4.1 公开数据集的 sparse embedding 存储"标尺"

> **用途**：在做"我的业务要花多少存储"估算时，先和公开数据集对比。

> **本节聚焦"业务场景 → sparse embedding 实际存储空间 → 工程方案"的三维映射**。
> 与"4.3 厂商 × 业务 × 规模真实落地数据"侧重线上业务指标不同，本节专门回答："**我这个业务到底要花多少存储？用什么工程方案解决？**"
...
### 4.2 不同业务场景的 sparse embedding 存储空间估算

> **本表基于"4.3 厂商 × 业务 × 规模真实落地数据" + "4.1 公开数据集标尺" + 工程经验值综合估算**。
> 估算逻辑：vocab_size × embedding_dim × bytes_per_value（精度）。


> **本表基于"4.3 厂商 × 业务 × 规模" + "4.1 公开数据集标尺" + "B.5 工程经验值"综合估算**。
> 估算逻辑：vocab_size × embedding_dim × bytes_per_value（精度）。

#### 4.2.1 短视频 / 内容推荐（字节 TikTok / 抖音 / 快手）
...
#### 4.2.4 社交网络 / 朋友圈（Meta / 微信 / LinkedIn）
...
#### 4.2.6 LLM 推荐 / Reasoning（OneReason / OpenOneRec）
...
#### 4.2.7 海外公司（Meta / YouTube / Shopee / Microsoft）
...
> **本节是工业级 sparse embedding 在各大厂实际生产中的真实规模 + 业务指标**。
> 数据点均通过知乎专栏一手抓取，并标注每个数据点的知乎作者 / 票数 / 可信度。

#### 4.3.1 厂商 × 业务 × sparse embedding 规模 · 核心矩阵（2025-2026）
...
#### 4.3.2 关键业务规模数字（按业务域重新组织）


#### 4.3.2.1 抖音（字节跳动）· 真实线上规模
...
#### 4.3.3.2 趋势二：sparse embedding 比例变迁
...
#### 4.4.1 NVIDIA Merlin / HugeCTR（工业级标杆）
...
#### 4.4.2 Meta TorchRec / Monolith（生产实践标杆）
...
#### 4.4.3 Meta HSTU / ULTRA-HSTU（2024-2026）
...
#### 4.4.4 OneFlow OneEmbedding（云端方案）
...
#### 4.4.5 阿里 PAI-HybridBackend / HybridEmbedding（开源标杆）
...
#### 4.4.7 微软 / Twitter / Meta Persia 极端方案（100T 参数级别）
...
#### 4.4.8 Hash Embedding 系列（理论压缩路线）
...
### 4.5 storage budget 估算公式 + 业界 4 大降本策略
...
### 4.7 知乎溯源清单 + 关键作者可信度评估

> **本节合并所有知乎信源**，按"作者可信度 + 内容主题"组织，避免分散在多个章节。

#### 4.7.1 关键作者可信度评估（核心来源）
...
| # | 数据源 | 类型 | 数量 | 强相关率 |
|---|--------|------|------|---------|
| 20 | **九老师关注者（反向）** | followers | 80 | 10% |
| 21 | **李博杰关注者（反向）** | followers | 80 | 1.3% |
| 22 | **王喆关注者（反向）** | followers | 100 | 6% |
| 23 | **SimmerChan关注者（反向）** | followers | 80 | 3.8% |
| 24 | **Q5 Embedding 压缩回答者** | answers | 16 | 37.5% |
| 25 | **Q7 LLM 推理加速回答者** | answers | 3 | 67% |

**v5 关键方法论发现**：反向网络（followers）信号噪声远大于正向网络（followees）—— 强相关率仅 ~3-10%，而正向网络 ~30-40%。但反向网络发现了"隐藏的一线工程师"：

| 用户 | people ID | 身份 | 来源 |
|------|----------|------|------|
| **双榆树周主任** | btty-23 | **字节跳动员工 / MLSys** | 九老师关注者 |
| **CaramelMacchiato** | yu-zong-75 | **哔哩哔哩算法工程师** | 王喆关注者 |
| **SparkShi** | sparkshi | **同济大学智能科学与技术博士** | SimmerChan关注者 |
| **童话闲谈** | 84-59-4-77-66-57 | **推荐算法资深从业者** | Q5 回答者 |
| **ZZZZJ** | ze-jia-zheng | **GPU @ RunwayML** | Q5 回答者 |

**v5 跨源升级**：王小新 (2→3 源)、deephub (2→3 源)
**v5 厂商新增**：哔哩哔哩（B站）/ RunwayML / 卓驭科技

**v4 新增 7 个数据源**（4-5 跳深度扩展）：

| # | 数据源 | 抓取 | 数量 |
|---|--------|------|------|
| 13 | **周源关注**（知乎 CEO）| `/api/v4/members/zhouyuan/followees` | **160** |
| 14 | **李沐关注**（亚马逊首席科学家）| `/api/v4/members/mli65/followees` | 26 |
| 15 | **苏剑林关注**（科学空间）| `/api/v4/members/su-jian-lin-22/followees` | 32 |
| 16 | **田渊栋关注**（前 MetaAI 总监）| `/api/v4/members/tian-yuan-dong/followees` | 60 |
| 17 | **强化学徒关注**（USTC 博士 / ChatPaper）| `/api/v4/members/heda-he-28/followees` | 60 |
| 18 | **Q3 工业界推荐模型**（问题 314773668）| `/api/v4/questions/314773668/answers` | 36 |
| 19 | **Q4 Deep Learning 推荐**（问题 20830906）| `/api/v4/questions/20830906/answers` | 72 |

**v4 关键发现 · 6 个跨 4 源超级节点**：

| 姓名 | people ID | 身份 | 出现源数 |
|------|----------|------|---------|
| **九老师** | nphard-79 | TikTok / 推荐算法 | **7 源**（v4 唯一）|
| **王喆** | wang-zhe-58 | 清华 / 广告推荐 ED | 4 |
| **代季峰** | jifengdai | MSRA / 计算机视觉顶流 | 4 |
| **苏剑林** | su-jian-lin-22 | 科学空间 / 数据挖掘 | 4 |
| **周国睿** | zhou-guo-rui-25 | 欢迎自荐 | 4 |
| **李博杰** | li-bo-jie | 中科大+MSRA 联培计算机博士 / AI 创业者 | 4 |

**v4 新增顶级大 V**（4-5 跳深度发现）：

| 类别 | 用户 | 身份 |
|------|------|------|
| **OpenAI** | **MIT奶茶店长** (boyuan99) | OpenAI 科学家 |
| **国际学术** | **Jinjie Ni 倪瑾杰** (ni-jin-jie-85) | **谷歌研究科学家** / 南洋理工博士 |
| **国际学术** | **许华哲 Harry** (hzhe-xu) | **UCB 计算机博士** |
| **国际学术** | **王梦迪** (wang-meng-di-7323) | **Princeton AI** |
| **国际学术** | **史唯艳** (shi-wei-yan-63) | **Northeastern AP / Dialogue** |
| **学术** | **汪玉** (wang-yu-61-18-82) | **清华电子系长聘教授** |
| **学术** | **游凯超** (youkaichao) | 清华软工博士 / 大模型推理 |
| **学术** | **陈天奇** (crowowrk) | **XGBoost 作者** |
| **学术** | **熊辰炎** (xiong-chen-yan) | **CMU AP** / 优秀答主 |
| **学术** | **Naiyan Wang** (naiyan-wang) | CS PhD / 机器学习与计算机视觉 |
| **工业** | **庞天宇** (pang-tian-yu) | **腾讯混元 Principal Scientist** |
| **工业** | **王琰** (wang-yan-56-80) | **腾讯 NLP&LLM 专家研究员** |
| **工业** | **周博洋** (guai-ren-63) | **微软员工 / MS 做 AI** |
| **工业** | **张亮** (imike) | **知乎联合创始人** |
| **学术** | **东水长** (wang-xi-huai) | 上交计算机博士 / 强化学习 |
| **学术** | **郑思泽** (zheng-si-ze-45) | 北大计算机系统结构博士 / UW 访问 |
| **学术** | **233** (233-26-96) | 清华计算机博士 |
| **学术** | **skydownacai** (skydownacai-9) | FDU PhD / RL theory / RL4LLM |
| **工业** | **稚晖** (zhi-hui-64-54) | 电子科大通信与信息系统硕士 |
| **工业** | **胡渊鸣 (iterator)** | 太极图形 CEO |
| **工业** | **陆平** (wishawk-72) | 苏黎世联邦理工博后 / RaBitQ 作者 |
| **工业** | **高飞** (gaofei) | **PEC 提示工程峰会发起人** |
| **教育** | **张春雨** (zsprain) | 资深 IT 出版 |
| **教育** | **曾加** (zengjiaplus) | 清华本硕 / 小红书 B 站"普拉思" |
...
| 字节跳动 | ✓✓ | (v3 已有) |
| 阿里巴巴 | ✓✓ | (v3 已有) |
| 腾讯 | ✓✓ | **庞天宇 (混元 Principal Scientist)** / **王琰 (NLP&LLM 专家)** |
| OpenAI | ✓ | **MIT奶茶店长 (OpenAI 科学家)** |
| 谷歌 | ✓✓ | **Jinjie Ni (谷歌研究科学家)** |
| 亚马逊 | ✓✓ | (v3 已有) |
| Meta | ✓✓ | (v3 已有) |
| 微软 | ✓✓ | **周博洋** / (v3 沪漂城哥) |
| 微软亚研院 | ✓ | (v3 已有) |
...
1. **6 个 4 源超级节点** 是图谱的"骨架" — 跨越学术 / 工业 / 跨学科
2. **4 跳深度发现** — 学术顶流（汪玉/UBC PhD/Princeton AI）、工业顶流（OpenAI/谷歌/微软/腾讯/混元）覆盖 5 跳以内
3. **周源关注 160 人** 含 60+ AI/科技，是 v4 最大的"用户信息源"
4. **强化学徒 (USTC 博士)** 关注链 = OpenAI + 腾讯混元 + 微软 三角
5. **方法升级**：从"互关对"升级到"互关网络 + 顶级大 V 跨域覆盖" — 6 源象限 + 4 跳深度

**v4 推荐 SimmerChan 加关注 Top 30**（从 20 扩展到 30）：

1-3. 浅梦/王喆/九老师 ✓ 已有
4-6. GuoXun / 0xC001 / 傅聪_Cong
7-9. **李博杰 / 代季峰 / 苏剑林** (4 源覆盖)
10-11. **李沐 (亚马逊首席科学家) / 周源 (知乎 CEO)**
12-13. **田渊栋 / MIT奶茶店长 (OpenAI)**
14-15. **Jinjie Ni (谷歌研究科学家) / 张亮 (知乎联合创始人)**
16-17. **陈天奇 (XGBoost 作者) / 游凯超 (清华软工博士/大模型推理)**
18-19. **熊辰炎 (CMU AP) / 汪玉 (清华电子长聘教授)**
20-21. **许华哲 (UCB PhD) / 王梦迪 (Princeton AI)**
22-23. **庞天宇 (腾讯混元 Principal Scientist) / 王琰 (腾讯 NLP&LLM 专家)**
24. **沪漂城哥 (微软算法专家)**
25-26. **林建入 (公众号) / DataMan (抖音 feed)**
27-28. **萧瑟 (王哲) / 俞扬 (南大人工智能学院教授)**
29-30. **黄海广 (机器学习初学者) / 陆平 (苏黎世联邦理工/RaBitQ)**
